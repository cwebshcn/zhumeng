...
<div style="width:1024px;margin:0 auto;text-align: center;padding:25px;">
	...
	<div class="row" >
		<div>&nbsp;</div>
	<?php 
		$result=$lnk -> query("select phone from spread_user where item_id='".$pid."' and phone>0 and open_status=1 order by rand()");
...
<?php 
	function count_all_phone($item_id){
		global $lnk;
		$result=$lnk -> query("select count(0) from spread_user where item_id='".$item_id."' and phone>0 and open_status=1 order by redeem_count_bc desc"); 
